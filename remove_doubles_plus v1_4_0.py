...
class Multi_OT_Merge(bpy.types.Operator):
    ...
    merge_center: BoolProperty(name="multi merge centre", default=True, update=update_center)
    merge_last: BoolProperty(name="multi merge last", default=False, update=update_last)
    merge_first: BoolProperty(name="multi merge first", default=False, update=update_first)
    ...
    def execute(self, context):
        ...
        def Merge(v1,v2):            
            ...
            if self.merge_last:
                history.add(v2)
                bpy.ops.mesh.merge(type='LAST', uvs=True) #'INVOKE_DEFAULT'
            if self.merge_first:
                history.add(v1)
